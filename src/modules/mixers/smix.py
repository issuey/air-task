...
class SelfWeightMixer(nn.Module):
    def __init__(self, args, observation_shape):
        super(SelfWeightMixer, self).__init__()
    ...
    def forward(self, agent_qs, states, observations, is_alive):
        ...
        observations = observations.reshape(-1,
                                            self.n_agents, observation_shape)
        ...
        state_hidden_dup = th.zeros(observation_hidden.shape).to(
            observation_hidden.device)
        ...
        q_tot = th.bmm(q.transpose(1, 2), k) / \
            is_alive.sum(1, keepdim=True).clamp(1, 8) + v
