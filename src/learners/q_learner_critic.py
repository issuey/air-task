...
from modules.mixers.smix import SelfWeightMixer
...
class QCriticLearner:
    def __init__(self, mac, scheme, logger, args):
        self.args = args
        self.mac = mac
        self.logger = logger
        observation_shape = scheme["obs"]["vshape"]
        ...
        if args.mixer is not None:
            if args.mixer == 'smix':
                self.mixer = SelfWeightMixer(args, observation_shape)
            else:
    ...
    def train(self, batch: EpisodeBatch, t_env: int, episode_num: int):
        ...
        # 为了后面的td(lambda)修改下面两行
        target_mac_out = th.stack(
            target_mac_out, dim=1)  # Concat across time[1:]

        # Mask out unavailable actions
        target_mac_out[avail_actions == 0] = -9999999  # [:, 1:]
        ...
        if self.args.double_q:
            ...
            cur_max_actions = mac_out_detach.max(dim=3, keepdim=True)[
                1]  # 修改mac_out_detach[:, 1:]
        ...
        if self.mixer is not None:
            ...
            target_max_qvals = self.target_mixer(target_max_qvals, batch["state"], batch["obs"],
                                                 is_alive)
            # 计算n步回报，在这里用了td_lambda
            targets = build_td_lambda_targets(rewards, terminated, mask, target_max_qvals,
                                              self.args.n_agents, self.args.gamma, self.args.td_lambda)

        # Calculate 1-step Q-Learning targetss
